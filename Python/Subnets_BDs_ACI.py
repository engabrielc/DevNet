...
def get_token(username, password):
    auth_url = "https://YOUR-APIC-IP/api/aaaLogin.json" # Ensure that the correct APIC IP is being utilized
    auth_payload = {
        "aaaUser": {
            "attributes": {
                "name": username,
...
bd_list = ["1", "2", "3"]  # Add your BDs to this list

# Function to get subnets using the token and list of BDs
def get_subnets_for_bds(token, bd_list):
    for bd in bd_list:
        url = f"https://YOUR-APIC-IP/api/node/mo/uni/tn-YOUR-TENANT/BD-BD-{bd}.json?rsp-subtree=full&rsp-subtree-class=fvSubnet" # Change the Tenant if necessary
